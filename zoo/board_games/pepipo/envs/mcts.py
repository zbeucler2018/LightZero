...
class TwoPlayersMCTSNode(MCTSNode):
    ...
    def value(self):
        ...
        wins, loses = (self._results[1], self._results[-1]) if self.parent.env._current_player == "player_1" else (self._results[-1], self._results[1])
    ...
    def expand(self):
        ...
        # The mcts_simulate_action() function returns a new environment which resets the board and the current player flag.
        next_simulator_env = self.env.mcts_simulate_action(action)
    ...
    def is_terminal_node(self):
        """
            Overview:
                This function checks whether the current node is a terminal node (game ended).
            Returns:
    ...
    def rollout(self):
        ...
        while True:
            game_won = current_rollout_env.game.check_winner("player_1") or current_rollout_env.game.check_winner("player_2")
            game_tied = current_rollout_env.game.check_tie("player_1") or current_rollout_env.game.check_tie("player_2")
            game_finished = game_won or game_tied
            if game_finished:
                break
            possible_actions = current_rollout_env.legal_actions()
            action = self.rollout_policy(possible_actions)
            current_rollout_env = current_rollout_env.mcts_simulate_action(action)

        reward = None
        if current_rollout_env.game.check_winner("player_1"):
            reward = 1
        elif current_rollout_env.game.check_winner("player_2"):
            reward = -1
        elif current_rollout_env.game.check_tie("player_1") or current_rollout_env.game.check_tie("player_2"):
            reward = 0
        return reward
